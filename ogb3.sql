...
clear screen
REM
REM NO MORE TRIGGERS
REM
pause
REM
REM create or replace
REM trigger customer_trg 
REM before insert on customers
REM for each row
REM begin
REM   if :new.customer_id is null then
REM      :new.customer_id := customers_seq.nextval;
REM   end if;
REM   
REM   if :new.created_by is null then
REM      :new.created_by := user;
REM   end if;
REM end;
REM 
REM 
REM create or replace
REM trigger orders_trg 
REM before insert on orders
REM for each row
REM begin
REM   if :new.order_id is null then
REM      :new.order_id := orders_seq.nextval;
REM   end if;
REM 
REM   if :new.order_datetime is null then
REM      :new.order_datetime := sysdate;
REM   end if;
REM   
REM   if :new.created_by is null then
REM      :new.created_by := user;
...
REM 
REM create or replace
REM trigger order_items_trg 
REM before insert on order_items
REM for each row
REM begin
REM   if :new.created_by is null then
REM      :new.created_by := user;
