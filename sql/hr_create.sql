...
DROP USER &acct CASCADE;
CREATE USER &acct IDENTIFIED BY &acct
/

ALTER USER &acct DEFAULT TABLESPACE &tbs QUOTA UNLIMITED ON &tbs;

GRANT CREATE SESSION, CREATE VIEW, ALTER SESSION, CREATE SEQUENCE TO &acct;
GRANT CREATE SYNONYM, RESOURCE TO &acct;

rem
rem START FROM HERE IF ALREADY CONNECTED AS HR
rem


alter session set current_schema = &acct;
...
EXECUTE dbms_stats.gather_schema_stats('&acct')
