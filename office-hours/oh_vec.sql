...
or pk between 120 and 123
or pk between 170 and 173
or pk between 220 and 223
or pk between 310 and 313;
pause
select * from trivia
...
where rownum <= 10;
pause 
select vector_embedding(doc_model using 'cocktail' as data) vec;
pause
...
select pk, facts
from trivia_vec
order by vector_distance(vec , vector_embedding(doc_model using 'greyhound' as data), cosine) 
fetch first 4 rows only;
pause
select pk, facts
from trivia_vec
...
pause
create or replace 
directory DOCS as '/home/oracle/docs';
pause
create table trivia_pdf
as select 123 pk, to_blob(bfilename('DOCS','trivia.pdf')) b ;
select dbms_lob.getlength(b) from trivia_pdf;
pause
clear screen
set long 200
select dbms_vector_chain.utl_to_text(b) 
from trivia_pdf
...
from trivia_pdf dt, 
     dbms_vector_chain.utl_to_chunks(
        dbms_vector_chain.utl_to_text(dt.b)) chunk
...
from 
  ...
  dbms_vector_chain.utl_to_embeddings(
    dbms_vector_chain.utl_to_chunks(
      dbms_vector_chain.utl_to_text(dt.b)),
...
pause
clear screen
create table trivia_chunked as
select dt.pk, et.*
from   trivia_pdf dt,
       dbms_vector_chain.utl_to_embeddings( 
              dbms_vector_chain.utl_to_chunks(
