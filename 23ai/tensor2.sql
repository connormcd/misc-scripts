...
from ( 
  select substr(key,1,1) x from bigrams
  union all
  select substr(key,2,1) from bigrams
)
where x != '~';
pause
select 1/58/58 from dual;
pause
clear screen
select key, 
       ratio_to_report(cnt) over ( ) as prob
from bigrams
order by 2
fetch first 10 rows only;
pause
clear screen
select sum(ln(prob)) lossiness
from 
(
select key, 
       ratio_to_report(cnt) over ( ) as prob
...
pause
select -sum(ln(prob)) nice_lossiness
from 
(
select key, 
       ratio_to_report(cnt) over ( ) as prob
